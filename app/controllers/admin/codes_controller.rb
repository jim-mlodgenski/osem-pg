  ...
  class CodesController < Admin::BaseController
    ...
    def create
      ...
      respond_to do |format|
        if @conference.save
          format.html { redirect_to admin_conference_codes_path, notice: 'Promo Code was successfully created.' }
        else
          format.html { redirect_to admin_conference_codes_path, error: "Oops, couldn't save Promo Code. #{@code.errors.full_messages.join('. ')}" }
        end
      end
    end
    ...
    def update
      if @code.update_attributes(code_params)
        ...
        redirect_to admin_conference_codes_path(conference_id: @conference.short_title), notice: "Update of promo code for #{@conference.short_title} failed. #{@code.errors.full_messages.join('. ')}"
      end
   end
