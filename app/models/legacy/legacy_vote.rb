class LegacyVote < LegacyBase
  ...
  def get_program
    Program.where(conference_id: self.conference_id).first
  end
  ...
  def map
    {
      ...
      abstract: self.description.split[0...500].join(' '),
  ...
  def migrate
    ...
    if self.conference_id == 4
      p 'found PGconf2017 proposal to be migrated'
      p self.id
      new_record = Event.new(map)
      # byebug
      ...
      if new_record.save!
        ...
        eu = EventUser.new(user_id: self.user_id, event_id: new_record.id, event_role: 'speaker')
        eu.save!
        # make the same user a submitter for the current event
        eu = EventUser.new(user_id: self.user_id, event_id: new_record.id, event_role: 'submitter')
        eu.save!
        # submitter must be registered to the conference
        @conference = get_program.conference
        @already_registered = Registration.where(user_id: self.user_id, conference_id: @conference.id).first
        unless @already_registered
          reg = Registration.new(user_id: self.user_id, conference_id: @conference.id, arrival: @conference.start_date, departure: @conference.end_date)
          reg.save!
        end
      end
    end
